...
| Render-target resize | Framework’s driver rebuilds canvas-scaled targets when canvas dimensions change and preserves fixed-size targets. This logic is headlessly tested. `packages/framework/src/render/brometal-driver.ts:183-213`; `packages/framework/src/render/brometal-driver.test.ts:155-161,348-363` | **Established:** reusable renderer resource behavior, not camera behavior. |
