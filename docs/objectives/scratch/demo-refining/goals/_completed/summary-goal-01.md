...
**Commits:** `93f18ab`, `e2d3962`, `bbc89ef`, `e1ebf4e`
...
## Three bugs found while building the harness
...
- **`capture_gameplay_sequence` is untouched.** Only still capture was unblocked. Anything about
  motion or feel — the camera shake work in goal 03 — still has no measurement path.
- **The blank-frame guard has not fired on real data.** Every live capture rendered. The three
  committed blank-white three.js captures the audit found are stale artifacts of a capture-timing
  bug, not a current render failure. Goal 10 owns removing them.
- **Budgets exist only for the four antiky demos.** The other six have committed metrics but no
  authored bounds.
- **The bounds are my proposal, not owner-approved art direction.** They are deliberately reachable
  but not trivial. If a demo lands a look you are happy with and its budget still fails, the budget
  is wrong and should be changed by you rather than by the agent failing it.
