...
|---|---|---|
| 1 | **The render driver is now a Current capability, not a Direction.** Both demos render through it. `packages/website/PRODUCT.md` and the site pages still say "accepted direction, not a current capability" — correct until now, wrong from this commit. Changing a public claim is yours, not mine. | product wording |
