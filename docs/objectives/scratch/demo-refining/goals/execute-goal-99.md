...
| G1 | **Where the shared town code lives.** `antiky-town` and `town-study` hold **4,889 byte-identical lines** across sixteen files, `character-motor.ts` among them. Goal 11 landed a drift guard (`packages/demos/tests/town-twin-parity.test.mjs`) but not the fix. | The shared home cannot be `@antiky/framework` while `town-study` is framework-free by the fence at `dev-host.test.mjs:72,95`. Moving that fence is a product decision. | The owner rules on the fence, or a third consumer appears and makes the copy indefensible. Detail in [`_completed/summary-goal-11.md`](_completed/summary-goal-11.md). |
