...
| A12 | **Peter-panning is unmeasured in point-light-expo.** Goal 06-04 requires the shadow's near edge within 4 px of the contact point; the metric built for it does not respond to bias, so it is not measuring peter-panning. | Rather than tune an instrument until it agreed, it was reported as invalid. Halving the bias from 0.03 to 0.012 moved the number not at all. | Whoever resumes 06-04. The same goal's frame-time budget is also untaken — it needs a live MCP session for `get_render_stats`. Detail in [`progress-goal-06-04.md`](progress-goal-06-04.md). |
