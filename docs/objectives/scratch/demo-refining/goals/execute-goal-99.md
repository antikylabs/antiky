...
| M5 | **`antiky-town` foliage is lit 2.5x darker than every other surface** (`src/town/index.ts:455`). Goal 03 documented it rather than changing it, as instructed. | Its origin commit states no reason, and the foliage shader already models leaf transmission, so this is a tuning offset rather than a lighting model. | Owner confirms whether it is deliberate; then the foliage goal. Suspect for the reported "horrid" trees. |
