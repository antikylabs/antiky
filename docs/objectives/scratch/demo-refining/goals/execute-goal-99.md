...
| A15 | **`sourceDigest` appears to hash gitignored build output.** `antiky-town` and `combat-arena` both fail their budgets on a stale digest after a session in which neither received a single tracked change. | Both directories carry gitignored `.antiky/` and `dist/` output that repeated `brometal prod` and dev-server runs rewrote. | Before the next capture sweep. As it stands every sidecar looks stale after any build, which trains people to re-shoot on a signal that means nothing — the opposite of what the digest is for. Confirm what the walk includes, then exclude build output. |
