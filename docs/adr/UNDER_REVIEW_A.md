...
### 13. Game-client host lifecycle and semantic input

**Why this is here:** The current Antiky Town demo loop does not define a reusable player-host lifecycle or meaningful input contract. **What it helps:** The boundary supports tests without graphics, replay, remapping, focus recovery, device loss, and prediction without letting browser frames control simulation. **Summary:** A thin host would own canvas and platform events. `EngineSession` would assign meaningful player inputs to fixed simulation steps, while `RenderDriver` owns graphics. **AI-suggested direction:** Keep the first boundary private to Town and record inputs with simulation-step identifiers. Promote it only for another host, public interface, or prediction.
