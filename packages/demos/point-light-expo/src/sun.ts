...
/**
 * The demo's one directional key light, and the shadow map it casts through.
 *
 * Everything here is derived from `RELAY_PRESENTATION.reliquaryBounds` rather than typed as
 * constants, so moving the scene moves the shadow map with it. That is required outcome 4 of goal
 * 06-04, and it is not ceremony: the numbers below are coupled to each other in ways that are easy
 * to break by hand and hard to see afterwards.
 */

/**
 * The direction light arrives *from*, normalised — the `L` in `dot(N, L)`.
 *
 * From the left and **from behind the scene**, at about 38 degrees of elevation. Both parts are
 * measured choices rather than taste:
 *
 * - **Behind.** The camera sits at `+z` (`presentation.ts`: position `[0, 18, 10.5]`), so a sun on
 *   the camera's side throws every shadow away from the viewer, where the caster hides it. Moving
 *   the sun to `-z` turns the shadows around to face the camera.
 * - **Low.** The first version of this sat at 59 degrees, and 89.8% of the floor came back fully
 *   lit — a high sun drops each prop's shadow underneath the prop. Elevation is what decides how
 *   much shadow the frame actually contains.
 *
 * It still crosses the three relay lights rather than running along them, so the sun models form
 * the point lights cannot: they sit at ground level inside their own rings and light little that is
 * not beside them.
 *
 * This is the demo's key direction, and `pipeline-invariants.test.mjs` asserts a demo has only one.
 * It reaches the shaders as a uniform, which is the shape `antiky-town` already uses.
 */
export const SUN_DIRECTION: readonly [number, number, number] = Object.freeze(
  normalise([-0.48, 0.62, -0.62]),
);
 ...
 * map would waste the difference. 2048 gives about 1.6 cm of world space per texel across this
 * scene, which is finer than the contact detail the probes measure.
 */
...
export function createRelaySunShadow(): SunShadow {
  const { minimum, maximum } = RELAY_PRESENTATION.reliquaryBounds;
  return createSunShadow(minimum, maximum);
}
