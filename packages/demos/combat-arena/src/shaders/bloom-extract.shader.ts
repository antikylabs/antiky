...
export default shader({
  ...
  fragment({ uScene, uThreshold, uTexel }, { vUv }) {
    // A real 4x downsample, not a point pick. One nearest tap per quarter-res texel read 1 of
    // every 16 scene pixels, so a two-pixel emissive line five times over white either vanished
    // or landed as one isolated hot texel — which the blur then walked outward as a lattice of
    // boxes. Four bilinear taps at quarter-texel offsets cover the full 4x4 footprint.
