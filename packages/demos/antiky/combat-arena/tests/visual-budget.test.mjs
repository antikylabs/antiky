 ...
 * spread 0.085, p95 0.085 — the darkest frame of the four.
 *
...
test('combat-arena frame reaches a real luminance spread', async () => {
...
test('combat-arena frame reaches genuine highlights', async () => {
  const metrics = await readMetrics();
  assert.ok(
...
test('combat-arena frame does not blow out its highlights', async () => {
  const metrics = await readMetrics();
  assert.ok(
...
test('combat-arena frame keeps its colour', async () => {
  const metrics = await readMetrics();
  assert.ok(
