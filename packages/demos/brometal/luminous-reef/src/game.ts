...
import luminousReefShader from './shaders/luminous-reef.shader.gen';

const game: StudioGameEntry = async ({ canvas, pointer, report }) => {
  const renderer = await createRenderer(canvas);
  const program = createProgram(renderer, luminousReefShader);
  ...
  report({ drawCalls: 1, instances: 1, uploadBytesPerFrame: 16, note: 'pure BroMetal reef shader' });
