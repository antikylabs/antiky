...
import type { AssetDownload, CatalogAsset, HashAlgorithm } from '../index.ts';
...
async function downloadFile(
  ...
  if (bytes.byteLength > MAX_DOWNLOAD_BYTES) throw new Error(`Asset file exceeds download limit: ${file.path}`);
  const actual = digest(file.hash.algorithm, bytes);
  if (actual !== file.hash.value.toLocaleLowerCase()) {
...
export async function installCatalogAsset(input: Readonly<{
  ...
  const installedFiles: Array<{
    path: string;
    sourceUrl: string;
    upstreamHash: AssetDownload['hash'];
    sha256: string;
    size: number;
  }> = [];
