...
function makeFixture(todo: string, feedback = "# Feedback\n\n---\n"): string {
  ...
  fixtures.push(root);
  mkdirSync(join(root, "docs", "objectives"), { recursive: true });
  writeFileSync(join(root, "docs", "objectives", "03-TODO_A.txt"), todo);
  writeFileSync(join(root, "docs", "objectives", "01-FEEDBACK_H.txt"), feedback);
  spawnSync("git", ["init", "-q"], { cwd: root });
  spawnSync("git", ["add", "docs/objectives/03-TODO_A.txt", "docs/objectives/01-FEEDBACK_H.txt"], {
    cwd: root,
  });
  spawnSync(
    "git",
...
function startTui(root: string, bin: string, wait = "20ms") {
...
`);
    ...
    expect(existsSync(join(root, "docs", "objectives", "04-AGENT-SESSIONS_A.txt"))).toBe(false);
  });
...
`);
  ...
  it("refuses to run a plan with no future checkpoint", () => {
    ...
    expect(result.stderr).toContain(
      "No open checkpoint remains in docs/objectives/03-TODO_A.txt; refusing to loop",
    );
  });
...
`,
    ...
    let todo = readFileSync(join(root, "docs", "objectives", "03-TODO_A.txt"), "utf8");
    ...
    const screen = result.stdout.replace(ansiPattern, "");
    ...
    const memory = readFileSync(
      join(root, "docs", "objectives", "04-AGENT-SESSIONS_A.txt"),
      "utf8",
    );
    expect(memory).toContain("# Testing summary\n\n- Check the completed task.");
    expect(memory).not.toContain("\u001b[");
    todo = readFileSync(join(root, "docs", "objectives", "03-TODO_A.txt"), "utf8");
...
`,
    ...
    expect(
      readFileSync(join(root, "docs", "objectives", "04-AGENT-SESSIONS_A.txt"), "utf8"),
...
writeFileSync(join(process.cwd(), "docs", "objectives", "01-FEEDBACK_H.txt"), "# Feedback\\n\\n---\\n");
...
`,
    ...
    expect(memory).toContain("# Antiky agent sessions");
    ...
    const todo = readFileSync(join(root, "docs", "objectives", "03-TODO_A.txt"), "utf8");
    expect(todo).toContain("x 2026-07-31 (A) 2026-07-30 First task");
    expect(todo).toContain("x 2026-07-31 (B) 2026-07-30 Second task");
...
`,
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain(
      "docs/objectives/03-TODO_A.txt did not change; stopping to avoid repeating the same task",
    );
