...
const ARCHIVER = resolve(import.meta.dirname, "../../archive-tasks.mjs");
...
function makeFixture(todo: string, archive?: string): string {
  ...
  if (archive !== undefined)
    writeFileSync(join(root, "docs", "objectives", "07-DONE_S.txt"), archive);
  return root;
}
...
`,
    ...
    expect(readFileSync(join(root, "docs", "objectives", "03-TODO_A.txt"), "utf8")).toBe(
      `# plan
${open}
${checkpoint}
`,
    ...
    const archive = readFileSync(join(root, "docs", "objectives", "07-DONE_S.txt"), "utf8");
...
`;
    ...
    expect(readFileSync(join(root, "docs", "objectives", "03-TODO_A.txt"), "utf8")).toBe(todo);
    expect(existsSync(join(root, "docs", "objectives", "07-DONE_S.txt"))).toBe(false);
  });
...
describe("the npm command names", () => {
  it("uses zd-prefixed names for both task runners", () => {
    ...
    expect(packageJson.scripts.zdloop).toBe("node packages/zdloop/session-loop.mjs");
    expect(packageJson.scripts.zdarchive).toBe("node packages/zdloop/archive-tasks.mjs");
