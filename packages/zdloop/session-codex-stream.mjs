...
export function runCodex(prompt, { activitySummarizer, approvalPolicy, control, sandbox, tui }) {
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    function handleJsonLine(line) {
      ...
      try {
        ...
        else activitySummarizer.push(line, update.activity);
        if (update.finalMessage !== undefined) finalMessage = update.finalMessage;
      } catch {
        activitySummarizer.push(line, `READ OUTPUT | The agent emits ${shorten(line, 80)}.`);
      }
    }
    ...
    if (child.stderr) {
      ...
      child.stderr.on("data", (chunk) => {
        consumeJsonLines(chunk, stderrStream, (line) => {
          ...
          activitySummarizer.push(
            line,
            `READ ERROR | The agent emits ${shorten(line, 80)} on stderr.`,
          );
        });
      });
    }
    ...
    child.on("close", async (code, signal) => {
      ...
      if (tui && !control?.killRequested) await activitySummarizer.flush();
