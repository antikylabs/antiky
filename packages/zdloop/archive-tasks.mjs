...
const ARCHIVE_RELATIVE_PATH = "docs/objectives/07-DONE_S.txt";
const TODO_PATH = resolve(TODO_RELATIVE_PATH);
const ARCHIVE_PATH = resolve(ARCHIVE_RELATIVE_PATH);
const ARCHIVE_HEADER = `# Antiky — completed tasks, moved out of ${TODO_RELATIVE_PATH} by /archive
...
function main() {
  try {
    if (process.argv.includes("--help")) {
      console.log(
        `Usage: npm run zdarchive\n\nMove every ^x line from ${TODO_RELATIVE_PATH} to ${ARCHIVE_RELATIVE_PATH}.`,
      );
      return;
    }
