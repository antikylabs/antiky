---
description: Move completed Antiky tasks into 07-DONE_S.txt, changing nothing else
...
`docs/objectives/03-TODO_A.txt` is the plan and the session log in one file, and finished lines are
never deleted. That is right, and it also means the file grows forever. Archiving keeps both: the
log survives in `docs/objectives/07-DONE_S.txt`, while the live plan stays short.

## What to do

1. **Count first.** `grep -c '^x ' docs/objectives/03-TODO_A.txt` and `grep -vc '^x ' docs/objectives/03-TODO_A.txt`. State both
...
3. Append them **verbatim and in file order** to `docs/objectives/07-DONE_S.txt`, under a `## <today>`
   heading (`date +%F`). Create the file with this header if it does not exist:

   ```
   # Antiky — completed tasks, moved out of docs/objectives/03-TODO_A.txt by /archive
   # Same format as the task list. Newest block last. Nothing here is edited or deleted.
   ```

4. Remove exactly those lines from `docs/objectives/03-TODO_A.txt`. Everything else stays untouched and in its
...
- Open-task count is identical to step 1. **This is the check that matters**: if it moved, you
  deleted live work — `git checkout docs/objectives/03-TODO_A.txt` and stop.
- Lines added to `docs/objectives/07-DONE_S.txt` equal lines removed from `docs/objectives/03-TODO_A.txt`.
- `grep -c '^x ' docs/objectives/03-TODO_A.txt` is 0, or exactly the finished lines a filter excluded.
