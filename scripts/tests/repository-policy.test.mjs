...
test('repository-level scripts stay within the owned allowlist', async () => {
  ...
  assert.deepEqual(tracked.split('\n'), [
    ...
    'scripts/tests/release-automation.test.mjs',
...
test('root package commands stay within the cross-workspace allowlist', async () => {
  ...
  assert.deepEqual(Object.keys(rootPackage.scripts).sort(), [
    ...
    'build',
    'check',
    'demos:assets:bake-sh9',
    'demos:assets:bake-vertex-occlusion',
    'demos:assets:build-detail-normal',
    'demos:assets:build-kit-materials',
    'demos:assets:build-lighting-ramp',
    'demos:assets:build-texture-atlas',
    'demos:assets:build-vfx-billboard',
    'demos:assets:embed-glb-images',
    'demos:assets:install-kenney',
    'demos:assets:install-nasa',
    'demos:assets:install-poly-haven',
    'demos:shoot',
    'demos:verify',
    'dev',
    'dev:demos',
