...
const runIdPattern = /^s00-\d{8}T\d{6}Z$/;
...
function validateStatuses(entries, label, errors) {
  ...
  for (const entry of entries) {
    ...
    } else if (entry.status === 'N/A' && !(typeof entry.reason === 'string' && entry.reason.length > 0)) {
...
export function validateReceipt(receipt) {
  ...
  if (receipt.sliceId !== 'slice-00') errors.push('Receipt sliceId must be slice-00.');
  ...
  if (!Array.isArray(receipt.rubric) || receipt.rubric.length === 0) {
    ...
    for (const row of receipt.rubric) {
      ...
      if (row?.status === 'N/A' && !(typeof row.reason === 'string' && row.reason.length > 0)) {
  ...
  if (!isRecord(receipt.afterCompletion)) errors.push('afterCompletion is required.');
    ...
    if (!(typeof receipt.afterCompletion[key] === 'string' && receipt.afterCompletion[key].length > 0)) {
...
export async function writeReceiptAtomic(file, receipt) {
  ...
  if (errors.length > 0) throw new Error(`Invalid Slice 00 receipt:\n${errors.join('\n')}`);
