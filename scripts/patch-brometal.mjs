 ...
 * dependency graph: BroMetal has been hoisted to the repository root in the past and is currently
 * nested inside each demo workspace. Patching only the first copy leaves the others unpatched and
 * the failure is silent — the demo simply renders with the unpatched runtime.
 */
...
for (packageRoot of installs) {
  const metadata = JSON.parse(await readFile(path.join(packageRoot, 'package.json'), 'utf8'));
  if (metadata.version !== EXPECTED_VERSION) {
    throw new Error(`Expected BroMetal ${EXPECTED_VERSION}, found ${metadata.version} at ${packageRoot}. Review the cut-out patch before upgrading.`);
  }
